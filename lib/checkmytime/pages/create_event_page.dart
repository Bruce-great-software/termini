...
class _CreateEventPageState extends State<CreateEventPage> {
  ...
  Future<void> _loadExistingEvent() async {
    ...
    try {
      ...
      setState(() {
        ...
        _eventType = (data['type'] ?? 'open').toString().trim().isEmpty
            ? 'open'
  ...
  Future<void> _submit() async {
    ...
    if (title.isEmpty) {
      _showMessage('Bitte gib einen Titel für das Event ein.');
      return;
    }
    ...
    try {
      ...
      if (_isEditMode) {
        ...
        final acceptedUserIds = List<String>.from(existingData['acceptedUserIds'] ?? const []);
        final maybeUserIds = List<String>.from(existingData['maybeUserIds'] ?? const []);
        final declinedUserIds = List<String>.from(existingData['declinedUserIds'] ?? const []);

        final cleanedAccepted = acceptedUserIds
            .where((id) => id == currentUser.uid || invitedUserIds.contains(id))
            .toList();
        final cleanedMaybe = maybeUserIds.where(invitedUserIds.contains).toList();
        final cleanedDeclined = declinedUserIds.where(invitedUserIds.contains).toList();
                ...
                children: [
                  ...
                  Text(
                    _isEditMode
                        ...
                        : 'Für den ersten Schritt erstellen wir ein einfaches offenes Event. Später kommen Einladungen, Teilnehmer und Zusagen dazu.',
                    style: theme.textTheme.bodyMedium?.copyWith(
                      color: colorScheme.onSurfaceVariant,
                    ),
                  ),
                ...
                child: filtered.isEmpty
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return CheckboxListTile(
                      ...
                      onChanged: (value) {
                        setState(() {
                          final updated = _users[originalIndex].copyWith(
